...
const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, history, onSelectSentence, t }) => {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                ...
                <p className="text-xs">{t('modal.history.empty_desc')}</p>
            </div>
          ) : (
            history.map((sentence, idx) => (
              <button
                ...
                className="w-full bg-white p-3 rounded-2xl border-2 border-slate-200 hover:border-primary hover:shadow-md transition-all group flex items-center gap-3 text-left"
              >
                <div className="flex -space-x-2 overflow-hidden py-1 px-1">
                    {sentence.slice(0, 5).map((item, i) => (
                        <div key={i} className="w-10 h-10 rounded-full border-2 border-white bg-slate-200 relative shadow-sm">
                             <img 
                                src={item.imageUrl} 
                ...
                <div className="p-2 bg-slate-50 rounded-full text-slate-400 group-hover:text-primary group-hover:bg-primary/10 transition-colors">
                    <RotateCcw size={16} />
                </div>
              </button>
            ))
